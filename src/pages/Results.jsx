...
function Results({ orderData, results, onBack, onEdit }) {
  ...
  const [plockedPallets, setPlockedPallets] = useState({}); // Track plocked (done) pallets: { palletIndex: Set<boxIndex> }
  ...
  const [plockedMixPallItems, setPlockedMixPallItems] = useState({}); // Track plocked mix pall items: { comboIndex: Set<artikelnummer> }
  const [plockedStandaloneMixItems, setPlockedStandaloneMixItems] = useState(new Set()); // Track plocked standalone mix items
  ...
  const handleToggleCheckedPallet = (palletIndex, boxIndex) => {
    setCheckedPallets(prev => {
      ...
      if (newChecked[palletIndex].has(boxIndex)) {
        newChecked[palletIndex].delete(boxIndex);
        if (newChecked[palletIndex].size === 0) {
          delete newChecked[palletIndex];
        }
      } else {
        newChecked[palletIndex].add(boxIndex);
      }
  ...
  const handleToggleAllPalletBoxes = (palletIndex, pallet) => {
    setCheckedPallets(prev => {
      ...
      let boxCount;
      if (pallet.isSingleSkvettpall) {
        boxCount = 1;
      } else if (pallet.palletBoxCounts) {
        boxCount = pallet.palletBoxCounts.length;
      } else {
  ...
  const handleToggleCheckedSkvettpall = (comboIndex, skvettpallIndex) => {
    setCheckedComboSkvettpalls(prev => {
      const newChecked = { ...prev };
      ...
      if (newChecked[comboIndex].has(skvettpallIndex)) {
        newChecked[comboIndex].delete(skvettpallIndex);
        if (newChecked[comboIndex].size === 0) {
          delete newChecked[comboIndex];
        }
      } else {
        newChecked[comboIndex].add(skvettpallIndex);
      }
  ...
  const handleToggleCheckedMixPall = (comboIndex, mixPallItems) => {
    // Toggle all mix items in the mix pall
    setCheckedMixPallItems(prev => {
      const newChecked = { ...prev };
      ...
      // Check if all mix items are already checked
      const allChecked = mixPallItems.every(item => newChecked[comboIndex].has(item.artikelnummer));
      ...
      if (allChecked) {
        ...
        mixPallItems.forEach(item => {
          newChecked[comboIndex].add(item.artikelnummer);
        });
      }
  ...
  const handleToggleCheckedMixItem = (comboIndex, artikelnummer) => {
    setCheckedMixPallItems(prev => {
      const newChecked = { ...prev };
      ...
      if (newChecked[comboIndex].has(artikelnummer)) {
        newChecked[comboIndex].delete(artikelnummer);
        if (newChecked[comboIndex].size === 0) {
          delete newChecked[comboIndex];
        }
      } else {
        newChecked[comboIndex].add(artikelnummer);
      }
  ...
  const handleToggleStandaloneMixItem = (artikelnummer) => {
    setCheckedStandaloneMixItems(prev => {
      const newChecked = new Set(prev);
      if (newChecked.has(artikelnummer)) {
        newChecked.delete(artikelnummer);
      } else {
  ...
  const handleTogglePlockedAllPalletBoxes = (palletIndex, pallet) => {
    setPlockedPallets(prev => {
      const newPlocked = { ...prev };
  ...
  const handleTogglePlockedComboPallet = (comboIndex) => {
    setPlockedComboSkvettpalls(prev => {
      const newPlocked = { ...prev };
      const combo = comboPallets[comboIndex];
  ...
  const handleClickComboMixPallProduct = (comboIndex, artikelnummer) => {
    // Find the combo and get the mix pall item by artikelnummer
    const combo = comboPallets[comboIndex];
    const mixPallSkvettpall = combo.skvettpalls.find(s => s.isMixPall);
    if (mixPallSkvettpall && mixPallSkvettpall.mixPallItems) {
      const mixItemIndex = mixPallSkvettpall.mixPallItems.findIndex(item => item.artikelnummer === artikelnummer);
      if (mixItemIndex !== -1) {
  ...
  const handleClickStashedComboMixPallProduct = (comboIndex, artikelnummer) => {
                                  ...
                                  return (
                                    <div key={mixIdx} className={`combo-product-line ${isMixItemEditing ? 'editing' : ''}`} style={{
                                      ...
                                      {isMixItemEditing ? (
                                        ...
                                        <>
                                          <span 
                                            onClick={() => handleClickStashedComboMixPallProduct(index, mixItem.artikelnummer)}
                                            style={{
                                              cursor: 'pointer',
                                              flex: 1,
                                              padding: '0.25rem',
                    ...
                    <div 
                      ...
                      onClick={(e) => {
                        if (!e.target.closest('.icon-btn')) {
                          handleTogglePlockedComboPallet(index);
                        }
                      }}
                      onDoubleClick={(e) => {
                        if (!e.target.closest('.icon-btn')) {
                      ...
                      title="Klicka för att plocka/avplocka alla skvettpalls, dubbelklicka för att markera/avmarkera"
                    >
                      ...
                      {[...combo.skvettpalls]
                        ...
                        return (
                          <div 
                            ...
                            title={!isEditing ? (isPlocked ? 'Klicka för att avplocka, dubbelklicka för att markera/avmarkera' : isChecked ? 'Klicka för att plocka, dubbelklicka för att avmarkera' : 'Klicka för att plocka, dubbelklicka för att markera') : undefined}
                          >
                            {isMixPall ? (
                              // Display mix pall items
                              <div 
                              ...
                              >
                                <div 
                                  ...
                                  title="Klicka för att plocka/avplocka alla, dubbelklicka för att markera/avmarkera alla"
                                >
                                  <span>Blandpall</span>
                                  <button 
                                    className="icon-btn" 
                                ...
                                {skvettpall.mixPallItems && [...skvettpall.mixPallItems].sort((a, b) => b.boxCount - a.boxCount).map((mixItem, mixIdx) => {
                                  ...
                                  return (
                                    <div 
                                      key={mixIdx} 
                                      className={`combo-product-line ${isMixItemEditing ? 'editing' : ''}`} 
                                      style={{
                                        fontSize: '0.8rem',
                                        padding: '0.25rem 0.5rem',
                                        marginBottom: '0.25rem',
                                        background: isMixItemChecked ? '#28a745' : '#f8f9fa',
                                        color: isMixItemChecked ? 'white' : undefined,
                                        borderRadius: '4px',
                                        display: 'flex',
                                        justifyContent: 'space-between',
                                        alignItems: 'center',
                                      ...
                                      title={!isMixItemEditing ? (isMixItemPlocked ? 'Klicka för att avplocka, dubbelklicka för att markera/avmarkera' : isMixItemChecked ? 'Klicka för att plocka, dubbelklicka för att avmarkera' : 'Klicka för att plocka, dubbelklicka för att markera') : undefined}
                                    >
                                      {isMixItemEditing ? (
                                        <>
                                          <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                ...
                <div className="combo-products">
                  {[...mixPall].sort((a, b) => b.boxCount - a.boxCount).map((item, index) => {
                    ...
                    return (
                      <div 
                        key={index} 
                        className={`combo-product-line ${isEditing ? 'editing' : ''}`} 
                        style={{
                          fontSize: '0.85rem', 
                          padding: '0.4rem',
                          background: isChecked ? '#28a745' : undefined,
                          ...
                          <>
                            ...
                            >
                              <strong 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleClickMixPallProduct(index);
                                }}
                                style={{cursor: 'pointer'}}
                                title="Klicka för att redigera"
                              >
                            ...
                            <button 
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteMixPallProduct(index);
                              }}
                              title="Ta bort"
                              style={{fontSize: '0.8rem', padding: '0.2rem'}}
                            >
                      ...
                      <td>
                        {editingPalletIndex === index ? (
                          ...
                          <span 
                            onClick={(e) => {
                              e.stopPropagation();
                              handleTogglePlockedAllPalletBoxes(index, pallet);
                            }}
                            onDoubleClick={(e) => {
                              e.stopPropagation();
                              handleToggleAllPalletBoxes(index, pallet);
                            }}
                            style={{cursor: 'pointer'}}
                            title="Klicka för att plocka/avplocka alla, dubbelklicka för att markera/avmarkera alla"
                          >
                            ...
                            {pallet.isSingleSkvettpall ? (
                              <>
                                <span 
                                  ...
                                  title={plockedPallets[index]?.has(0) ? 'Klicka för att avplocka, dubbelklicka för att markera/avmarkera' : checkedPallets[index]?.has(0) ? 'Klicka för att plocka, dubbelklicka för att avmarkera' : 'Klicka för att plocka, dubbelklicka för att markera'}
                                >
                              ...
                              pallet.palletBoxCounts.map((boxCount, i) => {
                                ...
                                const isPlocked = plockedPallets[index]?.has(i);
                                ...
                                return (
                                  <span 
                                    ...
                                    title={isPlocked ? 'Klicka för att avplocka, dubbelklicka för att markera/avmarkera' : isChecked ? 'Klicka för att plocka, dubbelklicka för att avmarkera' : 'Klicka för att plocka, dubbelklicka för att markera'}
                                  >
                                    {boxCount}
                                    <button
